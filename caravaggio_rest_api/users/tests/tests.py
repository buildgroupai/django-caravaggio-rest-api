...
class GetAllClientTest(CaravaggioBaseTest):
    ...
    def setUpTestData(cls):
        ...
        cls.manual_user_2 = cls.create_user(
            email="monica@mycompany.com",
            first_name="Monica",
            last_name="Belluci")

        current_path = os.path.dirname(os.path.abspath(__file__))
        cls.clients = GetAllClientTest.\
            load_test_data("{}/clients.json".format(current_path),
                           CaravaggioClientSerializerV1)

    def step1_create_clients(self):
        for client in self.clients:
            _logger.info("POST Client: {}".format(client["name"]))
            response = self.api_client.post(reverse("client-list"),
                                            data=json.dumps(
    ...
    def step2_get_clients(self):
        for index, client_id in enumerate(self.persisted_clients):
            ...
            self.assertEqual(
                response.data["name"], original_client["name"])
            super(GetAllClientTest, self).assert_equal_dicts(
                response.data, original_client,
                ["email", "id", "name", "is_active", "date_joined"])
    ...
    def step3_search_name(self):
        ...
        System 3
        """
        path = "{0}?name__icontains=system".\
            format(reverse("client-list"))
        _logger.info("Path: {}".format(path))
        response = self.api_client.get(path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        path = "{0}?name__icontains=system 3".\
            format(reverse("client-list"))
        _logger.info("Path: {}".format(path))
        response = self.api_client.get(path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # System 3 (position 3)
        self.assertEqual(
            response.data["results"][0]["name"], self.clients[2]["name"])
        super(GetAllClientTest, self).assert_equal_dicts(
            response.data["results"][0], self.clients[2],
            ["email", "id", "name", "is_active", "date_joined"])
