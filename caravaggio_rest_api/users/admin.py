...
from caravaggio_rest_api.users.models import \
    CaravaggioOrganization, CaravaggioClient, CaravaggioUser
from caravaggio_rest_api.users.forms import \
    CaravaggioUserCreationForm, CaravaggioUserChangeForm
...
class CaravaggioClientAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Important dates'), {
            'fields': ('date_joined', 'date_deactivated')}),
    )
...
class CaravaggioOrganizationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Users'), {
            'fields': ('owner', 'administrators', 'members',
                       'restricted_members'),
        }),
        (_('Permissions'), {
            'fields': ('is_active',),
    ...
    search_fields = (
        'id', 'email', 'name', 'owner', 'is_active', 'created', 'updated')
...
class CaravaggioUserAdmin(UserAdmin):
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('client', 'email', 'password1', 'password2'),
        }),
    )
