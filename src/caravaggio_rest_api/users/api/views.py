...
from caravaggio_rest_api.users.api.permissions import OrganizationUserAdminPermission
...
from caravaggio_rest_api.users.models import CaravaggioUser, CaravaggioClient, CaravaggioOrganization
from caravaggio_rest_api.users.api.serializers import (
    CaravaggioUserSerializerV1,
    CaravaggioClientSerializerV1,
    CaravaggioOrganizationSerializerV1,
    UserTokenSerializer,
)
from caravaggio_rest_api.users.api.permissions import ClientAdminPermission, OrganizationAdminPermission
...
class OrganizationViewSet(CaravaggioDjangoModelViewSet):
    ...
    filterset_fields = {
        ...
        "owner__is_client_staff": CaravaggioDjangoModelViewSet.BOOL_OPERATORS_ALL,
        "number_of_total_members": CaravaggioDjangoModelViewSet.NUMERIC_OPERATORS_ALL,
        "number_of_administrators": CaravaggioDjangoModelViewSet.NUMERIC_OPERATORS_ALL,
        "number_of_members": CaravaggioDjangoModelViewSet.NUMERIC_OPERATORS_ALL,
        "number_of_restricted_members": CaravaggioDjangoModelViewSet.NUMERIC_OPERATORS_ALL,
        ...
        "restricted_members__id": CaravaggioDjangoModelViewSet.PK_OPERATORS_ALL,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if obj.all_members.count() > 1:
            return Response(data={"message": "The organization still has members"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _process_add_user(self, collection, users):
        ...
        getattr(organization, collection).add(
            *CaravaggioUser.objects.filter(email__in=users, client=organization.client).all()
        )
        organization.save()
    ...
    def remove_administrator(self, request, pk):
        return self._process_remove_user("administrators", request.data["users"])

    @action(methods=["post"], detail=True)
    def add_administrator(self, request, pk):
        return self._process_add_user("administrators", request.data["users"])

    @action(methods=["post"], detail=True)
    def remove_member(self, request, pk):
        return self._process_remove_user("members", request.data["users"])

    @action(methods=["post"], detail=True)
    def add_member(self, request, pk):
        return self._process_add_user("members", request.data["users"])

    @action(methods=["post"], detail=True)
    def remove_restricted_member(self, request, pk):
        return self._process_remove_user("restricted_members", request.data["users"])

    @action(methods=["post"], detail=True)
    def add_restricted_member(self, request, pk):
        return self._process_add_user("restricted_members", request.data["users"])

    def get_queryset(self):
        if self.request.user.is_staff:
            return CaravaggioOrganization.objects.all()
        elif self.request.user.is_client_staff:
            return CaravaggioOrganization.objects.filter(client=self.request.user.client.id)
        else:
            return self.request.user.owner_of.union(self.request.user.administrator_of.all())
...
class UserViewSet(CaravaggioDjangoModelViewSet):
    ...
    filterset_fields = {
        ...
        "restricted_member_of__id": CaravaggioDjangoModelViewSet.PK_OPERATORS_ALL,
    }
    ...
    def get_queryset(self):
        if self.request.user.is_staff:
            ...
            return CaravaggioUser.objects.filter(client=self.request.user.client.id)
        else:
            # Get the organizations from which the user is owner or admin
            user_organizations = CaravaggioOrganization.objects.filter(
                id__in=self.request.user.owner_of.union(self.request.user.administrator_of.all()).values("id").all()
            )
            return CaravaggioUser.objects.filter(
                client=self.request.user.client.id, id__in=user_organizations.values("all_members")
            )
