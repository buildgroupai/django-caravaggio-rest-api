...
from caravaggio_rest_api.users.models import (
    CaravaggioClient,
    CaravaggioUserManager,
    CaravaggioOrganization,
    CaravaggioUser,
)
...
class CaravaggioClientSerializerV1(DynamicFieldsSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "email", "name", "is_active", "date_joined", "date_deactivated")
...
class CaravaggioOrganizationSerializerV1(DynamicFieldsSerializer):
    ...
    client = PrimaryKeyRelatedField(queryset=CaravaggioClient.objects.all(), required=True)

    owner = PrimaryKeyRelatedField(queryset=CaravaggioUser.objects.all(), required=True)

    administrators = PrimaryKeyRelatedField(queryset=CaravaggioUser.objects.all(), many=True, required=False)

    members = PrimaryKeyRelatedField(queryset=CaravaggioUser.objects.all(), many=True, required=False)

    restricted_members = PrimaryKeyRelatedField(queryset=CaravaggioUser.objects.all(), many=True, required=False)
...
class CaravaggioUserSerializerV1(DynamicFieldsSerializer):
    ...
    client = PrimaryKeyRelatedField(queryset=CaravaggioClient.objects.all(), required=False)
    ...
    def update(self, instance, validated_data):
        ...
        return super(CaravaggioUserSerializerV1, self).update(instance, validated_data)
    ...
    def validate_client(self, value):
        ...
        if request.user.client.id != value.id:
            raise serializers.ValidationError("You can only create users in your own Client space.")
...
class UserTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if email:
            user = CaravaggioUser.objects.get(username="{}-{}".format(client_id, email))

            # The authenticate call simply returns None for is_active=False
            # users. (Assuming the default ModelBackend authentication
            # backend.)
            if not user:
                msg = _("Unable to get the user with " "provided client id and email.")
