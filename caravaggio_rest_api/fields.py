...
class CassandraDateField(fields.DateField):
    ...
    def to_representation(self, value):
        ...
        return value.strftime(output_format) \
            if isinstance(value, datetime.date) \
            else value.date().strftime(output_format)
...
class DistanceField(fields.FloatField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert isinstance(self.units, str), (
            "The `units` argument must be an instance of `str`."
        )
        assert self.units in Distance.UNITS.keys(), \
            '`{}` invalid units.'.format(self.units)
