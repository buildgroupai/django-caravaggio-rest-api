...
class CompanyIndex(BaseSearchIndex, indexes.Indexable):
    ...
    round_notes = indexes.CharField(
        model_attr="round_notes")

    domain = indexes.CharField(
        model_attr="domain", faceted=True)

    foundation_date = indexes.DateField(
        model_attr="foundation_date", faceted=True)

    last_round = indexes.DateField(
        model_attr="last_round", faceted=True)

    country_code = indexes.CharField(
        model_attr="country_code", faceted=True)

    stock_symbol = indexes.CharField(
        model_attr="stock_symbol", faceted=True)
    ...
    founders = indexes.MultiValueField(
        null=True, model_attr="founders", faceted=True)
    ...
    crawler_config = indexes.CharField(
        model_attr="crawler_config")
    ...
    def prepare_autocomplete(obj):
        return " ".join((
            obj.name, obj.address_city, obj.address_zipcode
        ))
