...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--client", required=True, help="Specifies the ID of our Client (external system).",
        )
        parser.add_argument(
            "--email", help="Specifies the email for the new client.",
        )
        parser.add_argument(
            "--name", help="Specifies the name of the new Organization.",
        )
        parser.add_argument(
            "--owner",
            required=True,
            help="Specifies the ID of the user that will become the" " owner of the organization.",
        )
        parser.add_argument(
            "--noinput",
            "--no-input",
            ...
            if options["interactive"]:
                ...
                for field_name in CaravaggioOrganization.REQUIRED_FIELDS:
                    if options[field_name]:
                        field = CaravaggioOrganization._meta.get_field(field_name)
                        org_data[field_name] = field.clean(options[field_name], None)
                    else:
                        raise CommandError("You must use --%s with --noinput." % field_name)

            org_data["client"] = CaravaggioClient.objects.get(pk=org_data["client"])

            org_data["owner"] = CaravaggioUser.objects.get(pk=org_data["owner"])

            object = CaravaggioOrganization.objects.create(**org_data)

            if options["verbosity"] >= 1:
                self.stdout.write("Organization [{}] - [{}] " "created successfully.".format(object.id, object.name))
    ...
    def _validate_email(self, client_id, email, database):
        ...
        try:
            CaravaggioOrganization.objects.db_manager(database).get(client=client_id, email=email)
