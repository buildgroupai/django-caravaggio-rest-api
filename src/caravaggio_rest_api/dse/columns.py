...
class KeyEncodedMap(columns.Map):
    ...
    def to_python(self, value):
        ...
        if value is not None:
            return dict(
                (self.key_col.to_python(self.__keydecode__(k)), self.value_col.to_python(v)) for k, v in value.items()
            )
    ...
    def to_database(self, value):
        if value is None:
            return None
        return dict(
            (self.key_col.to_database(self.__keyencode__(k)), self.value_col.to_database(v)) for k, v in value.items()
        )
        ...
        if value is None:
            ...
            try:
                ...
                raise ValidationError(
                    ...
                    " IPv4 or IPv6 address for field".format((self.column_name or self.db_field), value)
                )
        return value
