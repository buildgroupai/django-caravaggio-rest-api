...
class CaravaggioClient(models.Model):
    ...
    name = models.CharField(_("client name"), max_length=100, null=False, blank=False)
...
class CaravaggioUser(AbstractUser):
    ...
    is_client_staff = models.BooleanField(
        ...
        default=False,
        help_text=_("Designates whether the user can operate with client users."),
    )
    ...
    EMAIL_FIELD = "email"
    REQUIRED_FIELDS = ["client", "email", "first_name", "last_name", "is_client_staff"]
...
class CaravaggioOrganization(models.Model):
    ...
    owner = models.ForeignKey(CaravaggioUser, related_name="owner_of", on_delete=PROTECT, blank=False, null=False)

    administrators = models.ManyToManyField(CaravaggioUser, related_name="administrator_of", blank=True)
    members = models.ManyToManyField(CaravaggioUser, related_name="member_of", blank=True)
    restricted_members = models.ManyToManyField(CaravaggioUser, related_name="restricted_member_of", blank=True)
    ...
    all_members = models.ManyToManyField(CaravaggioUser, related_name="organizations", blank=True)
...
def pre_save_client(sender, instance=None, using=None, update_fields=None, **kwargs):
...
def pre_delete_organization(sender, instance=None, using=None, **kwargs):
...
def pre_delete_user(sender, instance=None, using=None, **kwargs):
...
def pre_save_organization(sender, instance=None, using=None, update_fields=None, **kwargs):
...
def post_save_organization(sender, instance=None, created=None, using=None, update_fields=None, **kwargs):
    if created:
...
def compute_member_counters(instance):
    ...
    instance.number_of_restricted_members = instance.restricted_members.count()
...
def m2m_org_members_changed(signal, sender, **kwargs):
    ...
    if kwargs["pk_set"]:
        ...
        if action == "post_add":
            # do something when user was added
            for user in users:
                if user == organization.owner and user not in organization.administrators.all():
        ...
        if action == "post_add":
            ...
            for user in users:
                if (
                    ...
                    and user not in organization.administrators.all()
                ):
...
models.signals.m2m_changed.connect(m2m_org_administrators_changed, CaravaggioOrganization.administrators.through)
models.signals.m2m_changed.connect(m2m_org_members_changed, CaravaggioOrganization.members.through)
models.signals.m2m_changed.connect(
    m2m_org_restricted_members_changed, CaravaggioOrganization.restricted_members.through
)


# We need to set the new value for the changed_at field
@receiver(pre_save, sender=CaravaggioUser)
def pre_save_user(sender, instance=None, using=None, update_fields=None, **kwargs):
