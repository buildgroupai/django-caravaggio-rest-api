...
from drf_haystack.filters import HaystackOrderingFilter as DRFHaystackOrderingFilter

from drf_haystack.filters import HaystackFilter as DRFHaystackFilter

from drf_haystack.filters import HaystackFacetFilter as DRFHaystackFacetFilter

from drf_haystack.viewsets import HaystackViewSet

from rest_framework.filters import ORDER_PATTERN

from caravaggio_rest_api.drf_haystack.query import CaravaggioFilterQueryBuilder, CaravaggioFacetQueryBuilder
...
class CaravaggioHaystackFacetFilter(DRFHaystackFacetFilter):
    ...
    def apply_filters(self, queryset, applicable_filters=None, applicable_exclusions=None):
        ...
        queryset = super().apply_filters(
            queryset, applicable_filters=applicable_filters, applicable_exclusions=applicable_exclusions
        )
...
class HaystackOrderingFilter(DRFHaystackOrderingFilter):
    ...
    def get_valid_fields(self, queryset, view, context={}):
        ...
        if isinstance(view, HaystackViewSet):
            model_clazz = view.get_serializer_class()(context=context).Meta.model
            try:
                ...
                for field in valid_fields:
                    ...
                    if index_field and getattr(index_field, "faceted", False):
                        processed_valid_fields.append((field[0], "{}_exact".format(field[0])))
                    else:
    ...
    def remove_invalid_fields(self, queryset, fields, view, request):
        valid_fields = {item[0]: item[1] for item in self.get_valid_fields(queryset, view, {"request": request})}

        return [
            term.replace(term.lstrip("-"), valid_fields[term.lstrip("-")])
            for term in fields
            if term.lstrip("-") in valid_fields.keys() and (ORDER_PATTERN.match(term))
        ]
