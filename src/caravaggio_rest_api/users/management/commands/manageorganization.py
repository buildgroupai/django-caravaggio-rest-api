...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--id", required=True, help="Specifies the ID of the Organization to manage.",
        )
        parser.add_argument(
            "--relation",
            required=True,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for user_email in users_email:
                users.append(CaravaggioUser.objects.get(client=organization.client, email=user_email))
            ...
            self.stderr.write("\nUser do no exists in client [{}].".format(organization.client.id))
            sys.exit(1)
